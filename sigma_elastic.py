...
central_elastic_xs = centralU235.reactions[2].sigma
...
for i in file_indxs:
    ...
    elastic_xs = w180.reactions[2].sigma
    energy = w180.energy

    filename = 'csv_files/Godiva_elastic.csv'
    ...
    tmp = np.dot(sens_vec_values_adjusted,(elastic_xs.transpose()-central_elastic_xs.transpose()))
    if int(i)>=44:
...
plt.title('delta k_eff elastic_xs')
plt.xlabel('Values')
