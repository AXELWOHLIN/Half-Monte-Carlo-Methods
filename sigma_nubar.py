...
central_fission_xs = centralU235.reactions[mt("fission")].sigma
...
for i in file_indxs:
    ...
    fission_xs = w180.reactions[mt("fission")].sigma
    energy = w180.energy

    filename = 'csv_files/Godiva_fission.csv'
    ...
    tmp = np.dot(sens_vec_values_adjusted,(fission_xs.transpose()-central_fission_xs.transpose()))
    if int(i)>=44:
...
plt.title('delta k_eff fission_xs')
plt.xlabel('Values')
